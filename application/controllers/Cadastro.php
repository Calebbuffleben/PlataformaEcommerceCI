...
class Cadastro extends CI_Controller {
    ...
    public function adicionar() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->db->insert('clientes', $dados)) {
                ...
                echo 'Houve umm erro ao processar seu cadastro';
            }
        }
    }
    ...
    public function enviar_email_confirmacao($dados) {
        ...
        $this->email->from('calebbuffleben@gmail.com', "Hype Brazil");
        $this->email->to($dados['email']);
        $this->email->subject('Hype Brazil - Confirmação de cadastro');
    ...
    public function confirmar($hashEmail) {
        ...
        if ($this->db->update('clientes', $dados)) {
            ...
            echo 'Houve um erro ao confirmar o seu cadastro';
        }
    }
    ...
    public function login() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($autentica != NULL) {
                    $dadosSessao = $this->login->pegaDados();
    ...
    public function recuperar_login() {
        ...
        $senha = $this->input->post('senha');
        $autentica = $this->login->autenticar($usuario, $senha);
        $dados = $this->login->pegaDados();
        if ($autentica != NULL) {
            $mensagem = $this->load->view('emails/recuperar_senha.php', $dados, TRUE);
            $this->load->library('email');
            $this->email->from("calebbuffleben@gmail.com", "Hype - Brazil");
            $this->email->to($dados->email);
            $this->email->subject("Hype Brazil - Recupração de senha");
            $this->email->message($mensagem);
            if ($this->email->send()) {
                $data_header['categorias'] = $this->categorias;
                $this->load->view('html-header');
    ...
    public function meus_pedidos() {
        if (null != $this->session->userdata('logado')) {
            ...
            $this->db->where('cliente', $this->session->id);
